...
class DataValidation:
    ...
    def is_numerical_columns_exist(self, dataframe:pd.DataFrame) -> bool:
        """
        Checks if at least one numerical column exists in the dataframe.
        """
        try:
            numerical_columns = dataframe.select_dtypes(include=['int64', 'float64']).columns
            if len(numerical_columns) == 0:
                logging.warning("No numerical columns found in the dataframe.")
                return False
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not self.is_numerical_columns_exist(train_dataframe):
                raise NetworkSecurityException("No numerical columns found in training dataframe.", sys)
            if not self.is_numerical_columns_exist(test_dataframe):
                raise NetworkSecurityException("No numerical columns found in testing dataframe.", sys) 
